...
class ATT_GEN(nn.Module):
    def __init__(self, opt): 
        super(ATT_GEN, self).__init__()
    ...
    def forward(self, x, resv):

        h = self.tanh(self.fc1(x))
        m = self.tanh(self.fc2(resv))
        hm = self.tanh(h+m)
        out = self.softmax(self.fc3(hm))
        return h
...
class MLP_CRITIC(nn.Module):
    def __init__(self, opt): 
        ...
        self.fc0 = nn.Linear(opt.attSize,2048)
        self.fc1 = nn.Linear(opt.resSize + 2048, opt.ndh)
        #self.fc2 = nn.Linear(opt.ndh, opt.ndh)
